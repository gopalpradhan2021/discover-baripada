...
type Category = "food" | "places" | "culture" | "events" | "shopping";

type UniversalCategoryCardProps = {
  id: string;
  category: Category;
  title: string;
  image: string;
...
export function UniversalCategoryCard({
  id,
  category,
  title,
  image,
  ...
  venue,
  isBookmarked = false,
}: UniversalCategoryCardProps) {
  const { latitude, longitude } = useUserLocation();
  const { average } = useRatings(id);
  const displayRating = average ?? null;
  ...
  const showFoodType = category === "food";
  const showOpenStatus =
    category === "food" ||
    category === "places" ||
    category === "culture" ||
    category === "shopping";
  const showDistance = category !== "events";
  const showEventMeta = category === "events";
      ...
      <View style={styles.info}>
        ...
        <View style={styles.metaRow}>
          ...
          {showOpenStatus && (
            <View style={styles.metaItem}>
              <Ionicons
                ...
                size={10}
                color={openNow === null ? "#A7B0BD" : openNow ? "#3BD06B" : "#F0534F"}
              />
              <Text
                style={[
                  styles.metaText,
              ...
              >
                {openNow === null ? "Hours N/A" : openNow ? "Open" : "Closed"}
              </Text>
            </View>
          )}
          {showFoodType && (
          ...
          {showEventMeta && (
            <>
              <View style={styles.metaItem}>
                <Ionicons name="calendar-outline" size={14} color="#A7B0BD" />
                <Text style={styles.metaText}>{formatEventDate(eventDate)}</Text>
              </View>
              <View style={styles.metaItem}>
                <Ionicons name="location-outline" size={14} color="#A7B0BD" />
                <Text style={styles.metaText}>{venue ?? "Baripada"}</Text>
              </View>
            </>
          )}
        </View>
